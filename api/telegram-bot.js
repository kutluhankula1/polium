export default async function handler(req, res) {
    ...
    // Senin JSONBin Bilgilerin Eklenmiştir
    const BIN_ID = "6a8cf6a6da38895dfe0ce74c";
    const API_KEY = "$2a$10$G8jaeYrJCOhWdEhjmslybON9oM3pn6Lg8gnAODI5FzEBSc.foYKyS";

    // 1. GET İsteği: Sitedeki liderlik tablosu güncel listeyi çekmek istediğinde çalışır
    if (req.method === 'GET') {
        try {
            ...
            return res.status(200).json(data.record || { users: [] });
        } catch (err) {
            return res.status(500).json({ users: [] });
        }
    }

    // 2. POST İsteği (Formdan gelen başvuru)
    ...
    if (req.body && req.body.callback_query) {
        ...
        if (dataStr.startsWith('approve_')) {
            ...
            let currentUsers = binData.record.users || [];
